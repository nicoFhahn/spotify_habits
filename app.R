...
server <- function(input, output) {
  output$explanation <- renderText({
    if (input$plot_var == "Acousticness") {
      expl <- "This a confidence measure from 0.0 to 1.0 of whether the track
      is acoustic. 1.0 represents high confidence the track is acoustic."
    } else if (input$plot_var == "Danceability") {
      expl <- "Danceability describes how suitable a track is for dancing
      ...
      expl <- "Energy is a measure from 0.0 to 1.0 and represents a
      ...
      expl <- "Predicts whether a track contains no vocals. “Ooh” and
      ...
      represent instrumental tracks, but confidence is higher as
      the value approaches 1.0."
    } else if (input$plot_var == "Liveness") {
      expl <- "Detects the presence of an audience in the recording. Higher
      ...
      expl <- "The overall loudness of a track in decibels (dB). Loudness values
      ...
      expl <- "Speechiness detects the presence of spoken words in a track.
      ...
      expl <- "The overall estimated tempo of a track in beats per minute (BPM).
      In musical terminology, tempo is the speed or pace of a given piece and
      derives directly from the average beat duration."
    } else if (input$plot_var == "Valence") {
      expl <- "A measure from 0.0 to 1.0 describing the musical positiveness
      ...
      expl <- "The duration of the track in seconds."
    }
